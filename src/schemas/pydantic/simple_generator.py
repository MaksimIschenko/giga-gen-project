""" Pydantic schemas for Simple Generator """
...
class SimpleGeneratorRequest(BaseModel):
    ...
    - fewshot — true/false, добавлять ли демонстрационный 
                ответ ассистента для стабилизации стиля.
    ...
    style: str | None = Field(None, description="Опциональный system-промпт")
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                "prompt": "Простой логотип для компании, которая делает программное обеспечение для малых бизнесов", # noqa
                "mode": "logo",
                "fewshot": True,
